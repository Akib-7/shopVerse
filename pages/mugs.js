...
const Mugs = ({products}) => {
  ...
  return (
    ...
    <div className='mugs Section mx-3 w-fit grid-cols-2 grid md:grid-cols-3 lg:grid-cols-4 gap-y-4 md:gap-y-8 mt-5 md:mt-14  gap-x-3 md:gap-x-3 lg:gap-x-6 md:mx-6'>
      {/* Because products is an object and not an array we cant use map function simply. We will have to use OBJECT.Keys[products] */}
      {Object.keys(products).length===0 && <div className='justify-center items-center'><p className='  font-bold text-lg text-red-400'>Sorry, All the mugs are out of stock</p> <p className='block font-bold text-2xl'>New Stock coming soon! Stay Tuned</p></div>}
...
var products =await productModel.find({category:'mug'})

let mugs={}// it is an array of mugs with mugs's title is one element.
...
//Checikng if size/color is not an array we make it an array for storing different color,size variants of the proudcts which  hvae the same product title
    if(item.title in mugs){
        if (!Array.isArray(mugs[item.title].size)) {
            mugs[item.title].size = [];
        }
        if (!Array.isArray(mugs[item.title].color)) {
    ...
    // IF item is not present in the our mugs array we add that item in our mugs category and storing its size and color variant
    else{
  ...
  return {
    props: {products:JSON.parse(JSON.stringify(mugs)) },// deep copy because of object =_id
  }
}
