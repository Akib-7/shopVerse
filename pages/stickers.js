...
const Stickers = ({products}) => {
  ...
  return (
    ...
    <div className='stickers Section mx-3 w-fit grid-cols-2 grid md:grid-cols-3 lg:grid-cols-4 gap-y-4 md:gap-y-8 mt-5 md:mt-14  gap-x-3 md:gap-x-3 lg:gap-x-6 md:mx-6'>
      {/* Because products is an object and not an array we cant use map function simply. We will have to use OBJECT.Keys[products] */}
      {Object.keys(products).length===0 && <div className='justify-center items-center'><p className='  font-bold text-lg text-red-400'>Sorry, All the stickers are out of stock</p> <p className='block font-bold text-2xl'>New Stock coming soon! Stay Tuned</p></div>}
...
var products =await productModel.find({category:'sticker'})

let stickers={}// it is an array of stickers with stickers's title is one element.
...
//Checikng if size/color is not an array we make it an array for storing different color,size variants of the proudcts which  hvae the same product title
    if(item.title in stickers){
        if (!Array.isArray(stickers[item.title].size)) {
            stickers[item.title].size = [];
        }
        if (!Array.isArray(stickers[item.title].color)) {
    ...
    // IF item is not present in the our stickers array we add that item in our stickers category and storing its size and color variant
    else{
  ...
  return {
    props: {products:JSON.parse(JSON.stringify(stickers)) },// deep copy because of object =_id
  }
}
